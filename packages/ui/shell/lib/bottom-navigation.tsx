...
export function BottomNavigation(props: BottomNavigationProps) {
  ...
  return (
    <View className='flex-row items-center border-t border-neutral-800 bg-neutral-900 px-2 pt-1 pb-2'>
      <TabSlot
        item={props.tabs[0]}
        active={props.activeKey === props.tabs[0].key}
        onPress={props.onTabPress}
      />
      <TabSlot
        item={props.tabs[1]}
        active={props.activeKey === props.tabs[1].key}
        onPress={props.onTabPress}
      />
      <PrimaryAction label={props.primaryActionLabel} onPress={props.onPrimaryActionPress} />
      <TabSlot
        item={props.tabs[2]}
        active={props.activeKey === props.tabs[2].key}
        onPress={props.onTabPress}
      />
      <TabSlot
        item={props.tabs[3]}
        active={props.activeKey === props.tabs[3].key}
        onPress={props.onTabPress}
      />
    </View>
  )
...
function TabSlot(props: {
  ...
  return (
    ...
    >
      <Text
        className={
          props.active ? 'text-sm font-semibold text-pink-400' : 'text-sm text-neutral-500'
        }
      >
        {props.item.label}
      </Text>
  ...
  return (
    <Pressable onPress={props.onPress} className='flex-1 items-center active:opacity-80'>
      <View className='-mt-6 size-12 items-center justify-center rounded-full bg-pink-500 shadow-lg shadow-pink-500/40'>
        <Text className='text-2xl leading-9 font-bold text-white'>+</Text>
      </View>
